...

            {
                foreach (var item in type.GetAllProperties())
                {
                    ...
                    */
                    ...
                    var name = nameMutetor(item.Name);//(dm != null && dm.Name != null) ? dm.Name : nameMutetor(item.Name);
                    ...
                    {
                        throw new InvalidOperationException("same (custom)name is in type. Type:" 
                            + type.Name + " Name:" + member.Name);
                    }
