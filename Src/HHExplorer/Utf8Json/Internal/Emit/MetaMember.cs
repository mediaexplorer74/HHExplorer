...
namespace Utf8Json.Internal.Emit
{
    internal class MetaMember
    {
        ...
        static MethodInfo GetShouldSerialize(MemberInfo info)
        {
            ...
            //FIXIT
            // public only
            return info.DeclaringType.GetTypeInfo().GetDeclaredMethods("public")//(BindingFlags.Instance | BindingFlags.Public)
    ...
    {
        //FIXIT (true)
        static readonly MethodInfo getInnerException = ExpressionUtility.GetPropertyInfo((Exception ex) => ex.InnerException).GetGetMethod(true);
