...
namespace Xamarin.Essentials
{
    public static partial class WebAuthenticator
    {
        static async Task<WebAuthenticatorResult> PlatformAuthenticateAsync(WebAuthenticatorOptions webAuthenticatorOptions)
        {
            ...
            //        $"You need to declare the windows.protocol usage of the protocol/scheme `{callbackUrl.Scheme}` in your AppxManifest.xml file");

            try
            {
                var r = await WebAuthenticationBroker.AuthenticateAsync(
                    WebAuthenticationOptions.None, url, callbackUrl);
        ...
        {
            XDocument doc = XDocument.Load(Platform.AppManifestFilename, LoadOptions.None);
            ...
            IEnumerable<XElement> decl = default;//doc.Root.XPathSelectElements(
