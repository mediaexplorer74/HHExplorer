    ...
    public static partial class AppActions
    {
        ...
        static string ArgumentsToId(string arguments)
        {
            if (arguments?.StartsWith(appActionPrefix) ?? false)
                return Encoding.UTF8/*.Default*/.GetString(Convert.FromBase64String(arguments.Substring(appActionPrefix.Length)));
        ...
        {
            var id = appActionPrefix + Convert.ToBase64String(Encoding.UTF8/*.Default*/.GetBytes(action.Id));
