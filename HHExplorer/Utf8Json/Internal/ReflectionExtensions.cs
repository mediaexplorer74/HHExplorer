...
namespace Utf8Json.Internal
{
    internal static class ReflectionExtensions
    {
        ...
        static IEnumerable<PropertyInfo> GetAllPropertiesCore(Type type, HashSet<string> nameCheck)
        {
            ...
            {
                foreach (var item in GetAllPropertiesCore(type.GetTypeInfo().BaseType, nameCheck))
                {
                    yield return item;
                }
            }
        ...
        static IEnumerable<FieldInfo> GetAllFieldsCore(Type type, HashSet<string> nameCheck)
        {
            ...
            if (type.GetTypeInfo().BaseType != null)
            {
                foreach (var item in GetAllFieldsCore(type.GetTypeInfo().BaseType, nameCheck))
                {
                    yield return item;
                }
            }
        ...
        public static MethodInfo GetGetMethod(this PropertyInfo propInfo, bool v)
        {
            return propInfo.GetMethod;
        }

        public static MethodInfo GetGetMethod(bool v)
        {
            return null;
        }

        public static MethodInfo GetSetMethod(this PropertyInfo propInfo, bool v)
        {
            return propInfo.SetMethod;
        }

        public static MethodInfo GetSetMethod(bool v)
        {
            return null;
        }
