    ...
    public abstract class DynamicCompositeResolver : IJsonFormatterResolver
    {
            ...
            IJsonFormatterResolver[] resolvers)
        {
            ...
            var f = cacheType.DefineField("formatter", typeof(IJsonFormatter<>).MakeGenericType(genericP), FieldAttributes.Static | FieldAttributes.Public);
            {
                ...
                il.EmitCall(typeof(DynamicCompositeResolver)
                    .GetMethod("GetFormatterLoop").MakeGenericMethod(genericP));
            ...
            {
                var ctor = resolverType.DefineConstructor(MethodAttributes.Public, 
                    CallingConventions.Standard, new[] { typeof(IJsonFormatter[]), typeof(IJsonFormatterResolver[]) });
            ...
            */
            ...
            var resolverType = assembly.DefineType("DynamicCompositeResolver_" + id, 
                TypeAttributes.Class | TypeAttributes.Public | TypeAttributes.Sealed, typeof(DynamicCompositeResolver));
            var resolverT = resolverType.CreateTypeInfo().AsType();
            var instance = Activator.CreateInstance(resolverT, new object[] { formatters, resolvers });
