...
#if NETSTANDARD
            isBoxed = false;
            var dynamicMethod = new DynamicMethod("EnumSerializeByUnderlyingValue", null, new[] { 
...
#if NETSTANDARD
            isBoxed = false;
            var dynamicMethod = new DynamicMethod("EnumDeserializeByUnderlyingValue", type, new[] 
            { typeof(JsonReader).MakeByRefType(), typeof(IJsonFormatterResolver) }, 
            type.GetTypeInfo().Module, true);
...
namespace Utf8Json.Formatters
{
    ...
    public class EnumFormatter<T> : IJsonFormatter<T>, IObjectPropertyNameFormatter<T>
    {
        ...
        static EnumFormatter()
        {
            ...
            var type = typeof(T);
            foreach (var item in type.GetTypeInfo().DeclaredFields.Where(fi => fi.FieldType == type))
            {
